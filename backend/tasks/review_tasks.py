...
def run_review_task(
    ...
    try:
        ...
        async def _run_langgraph():
            ...
            config = {
                "configurable": {"thread_id": review_id, "github_token": github_token}
            }
        ...
        if exec_status == "paused":
            ...
            risk_score = consensus_res.get(
                "risk_score", _compute_risk_score(final_state.get("agent_findings", []))
            )
            ...
            asyncio.run(
                _update_review_status(
                    review_id, "paused_for_review", risk_score=risk_score
                )
            )
            logger.warning(
                "Workflow paused for Human-in-the-Loop review", review_id=review_id
            )
        ...
        if final_state.get("error"):
            asyncio.run(
                _update_review_status(review_id, "failed", error=final_state["error"])
            )
        ...
        risk_score = consensus_result.get(
...
async def _update_review_status(
    review_id: str, status: str, error: str = None, risk_score: int = None
):
        ...
        async with AsyncSessionLocal() as session:
            ...
            if pr and pr.user_id:
                ...
                if account and account.access_token_encrypted:
                    ...
                    raise Exception(
                        "GitHub Connector not configured. Cannot generate artifacts."
                    )
        return None
