...
with open("backend/api/routes/reviews.py", "r") as f:
    content = f.read()

# I will just write a new file instead to avoid massive regex complexity
new_content = """\
\"\"\"
...
async def get_review_status(review_id: str, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # Ensure a user can only view their own reviews
...
async def post_finding_to_pr(
    ...
    token_stmt = (
        ...
        .where(ThirdPartyUserAccount.user_id == current_user.id, ThirdPartyIntegration.provider_name == "github")
    )
    github_account = (await db.execute(token_stmt)).scalar_one_or_none()
    github_token = github_account.access_token if github_account else settings.github_token
    ...
    try:
        url = await post_single_finding_comment(pr.pr_url, finding_dict)
    finally:
        if old_token:
...
async def approve_review(review_id: str, request: ApproveReviewRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # Note: user_id is passed if generate_artifacts_task expects it, checking later
...
with open("backend/api/routes/reviews.py", "w") as f:
    f.write(new_content)
